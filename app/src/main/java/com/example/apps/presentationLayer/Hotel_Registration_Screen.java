                ...
                if (hotelName_string.equals("") || hotelAdd_string.equals("") ||
                    ...
                    try {
                        ...
                        if (hrs.validateHotel(hotelName_string, hotelLoc_string)) {
                            ...
                            cv.put(MyDBHelper.COLUMN_NAME,hotelName_string);
                            cv.put(MyDBHelper.COLUMN_LOCATION,hotelLoc_string);
                            cv.put(MyDBHelper.COLUMN_ADDRESS,hotelAdd_string);
                            cv.put(MyDBHelper.COLUMN_SINGLE_ROOMS,totalSingleRooms_string);
                            cv.put(MyDBHelper.COLUMN_DOUBLE_ROOMS,totalDoubleRooms_string);
                            cv.put(MyDBHelper.COLUMN_SINGLE_PRICE,singleRoomPrice_string);
                            cv.put(MyDBHelper.COLUMN_DOUBLE_PRICE,doubleRoomPrice_string);
                            cv.put(MyDBHelper.COLUMN_REGISTERED_BY,email);
                            double tep = database.insert(MyDBHelper.TABLE_HOTELS,null,cv);
                            database.close();
                            helper.close();
                            onBackPressed();
                        }
